...
function MonsterAttackForm({ attacks, onChange, onAddAttack, onRemoveAttack }) {
  ...
  return (
    <>
      ...
      {attacks.map((attack, index) => {
        // Count the number of each category up to this index
        let thisTypeNumber = 0;
        const category = getAttackCategory(attack);
        for (let i = 0; i <= index; i++) {
          if (getAttackCategory(attacks[i]) === category) {
            thisTypeNumber++;
          }
        }
        let header = `Attack ${index + 1}`;
        if (category === 'melee') header = `Melee Attack ${thisTypeNumber}`;
        else if (category === 'ranged') header = `Ranged Attack ${thisTypeNumber}`;
        else if (category === 'spell') header = `Spell ${thisTypeNumber}`;
        else if (category === 'regularSpell') header = `Regular Spell ${thisTypeNumber}`;
        return (
          <Card key={index} className="mb-3">
            <Card.Body>
              ...
              {(category !== 'spell' && category !== 'regularSpell') && (
                <>
                  <Row>
                    <Col md={6}>
